...
INSERT INTO EMPLOYEES (SIN,DOB,DOJ,EMAIL,PASSWORD) VALUES (0,'0001-10-01'::DATE,'0001-01-01'::DATE,0::VARCHAR,0::VARCHAR);
...
CREATE OR REPLACE PROCEDURE UPD_CUS (ID INTEGER,NAMEC character varying,STD INTEGER,ADDR character varying,EML IN CUSTOMERS.EMAIL%TYPE,PH1 IN CUS_PHONE.PH%TYPE)
...
UPDATE CUSTOMERS SET CNAME=NAMEC,ADDRESS=ADDR,STORE_ID=STD,EMAIL=EML WHERE CUS_ID=ID;
...
CREATE OR REPLACE PROCEDURE UPD_BY_CUS (ID INTEGER,NAMEC character varying,STD INTEGER,ADDR character varying,EML IN CUSTOMERS.EMAIL%TYPE,PAS IN CUSTOMERS.PASSWORD%TYPE,PH1 IN CUS_PHONE.PH%TYPE)
language plpgsql    
as $$
declare
...
UPDATE CUSTOMERS SET CNAME=NAMEC,ADDRESS=ADDR,STORE_ID=STD,EMAIL=EML,PASSWORD=PAS WHERE CUS_ID=ID;
UPDATE CUS_PHONE SET PH=PH1 WHERE CUS_ID=ID;
ELSE
RAISE NOTICE 'STORE NOT FOUND';
END IF;
...
CREATE OR REPLACE PROCEDURE UPD_TAPE (TPID integer,STK integer,PR integer,SID integer,TIT character varying,DIR character varying,DES character varying,GEN character varying,RAT integer,STR character varying,LINK character varying)
...
UPDATE MOVIES SET TITLE=TIT,DIRECTOR=DIR,GENRE=GEN,DESCRIPTION=DES,RATING=RAT,NOS=STR,IMDB=LINK WHERE TAPE_ID = TPID;
...
CALL ADD_PAY(1,1010,15,'PENDING','CASH',0);
